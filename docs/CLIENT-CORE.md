...
benchmark APK (`./codedeck apk benchmark --features native-core`) builds and
installs. What's left before the F2b stop-point is no longer code: a full
manual smoke pass on a real device, then deleting `src/core`. Two things
