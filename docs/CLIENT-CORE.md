...
| `machines` — `mergeSessionList` (bug-B) + title helpers (pure half) | `apps/mobile/src/core/stores/machines.ts` | `client_core::stores::machines` | ✅ F2a — `merge_session_list` (absence never deletes, tombstones only, `machineOffline`, stale grace, title guard, per-session extras survive), `title_from_first_message`, `prune_dismissed`. 15 tests incl. the 200×30 property run. The `MachineView` + `apply*` actions + serialize/hydrate = next F2a increment |
| stores (rest), sync, outbox, pairing, notifications, presentation, dm/marmot | `apps/mobile/src/core/**` | `client_core::**` | ⏳ F2a |
