...
| Re-point `apps/mobile` at the Rust `Core` + delete `src/core` | `createPhoneCore.ts` consumers | `apps/mobile/src` | ⏳ F2b — in progress, store by store: `createNative{Outbox,Settings,Pairing,Machines,Dm,Marmot,QuickPrompts,PendingSessions,Ui}Store` (`apps/mobile/src/core/stores/native*.ts`) are done, each presenting the exact public shape its local-reducer counterpart does, backed by `dispatch`/`*_view()`/`onCoreEvent` instead of a direct socket. `identity.ts` needs no adapter — the TS side always owns loading/generating the persisted secret locally (it is the `core_init` *argument*, not something Rust exposes back), identically in native and non-native mode. Left: the final swap of `createPhoneCore.ts` itself to pick the native store set, then deleting `src/core` only after `./codedeck check` + a full manual smoke pass are green against the re-pointed app. One documented gap surfaced along the way: sending `set-credentials`/`set-device-config`/`set-provider-profile` has no `Intent` yet (only receiving each ack does), so `nativeUi.ts`'s `note*Sent` optimistic "saving…" mutators are no-ops until that command-send surface is ported. |
