...
| Re-point `apps/mobile` at the Rust `Core` + delete `src/core` | `createPhoneCore.ts` consumers | `apps/mobile/src` | ⏳ F2b — in progress, store by store: `createNative{Outbox,Settings,Pairing,Machines,Dm,Marmot,QuickPrompts,PendingSessions}Store` (`apps/mobile/src/core/stores/native*.ts`) are done, each presenting the exact public shape its local-reducer counterpart does, backed by `dispatch`/`*_view()`/`onCoreEvent` instead of a direct socket. `identity.ts` needs no adapter — the TS side always owns loading/generating the persisted secret locally (it is the `core_init` *argument*, not something Rust exposes back), identically in native and non-native mode. Left: `ui.ts` (selection + optimistic interaction-card state) — blocked on a `CardsView` that does not exist yet (plan §2.1 lists it explicitly as landing "with the loop integration," not part of F2b's initial surface) — and the final swap of `createPhoneCore.ts` itself to pick the native store set, then deleting `src/core` only after `./codedeck check` + a full manual smoke pass are green against the re-pointed app. |
