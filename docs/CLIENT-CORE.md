...
| Rust integration test against the harness + re-point `apps/mobile` + delete `src/core` | `createPhoneCore.ts` consumers | `client_runtime` bindings | ⏳ F2b — the harness above exists; still needed: a `cargo test` that spawns it (`node tools/contract-harness/out/main.js`) and drives `client_runtime::Core` through pair → session → input → ack → reconnect → sync over the real socket (the F2b Capa 2 Go/No-Go gate), then rewriting `apps/mobile/src-tauri/src/corebridge.rs` to expose the full View/Intent/CoreEvent surface as Tauri commands, switching `apps/mobile/src` to consume it, and deleting `src/core` last, after green. |
...
`tools/contract-harness/` (Node) exists: a real `BridgeCore` + `FakeSdkFacade`
over a genuine `ws://` socket, driven by a documented stdin/stdout control
protocol — no FFI shim. Remaining: the `cargo test` that spawns it and drives
`client_runtime::Core` through the Capa 2 Go/No-Go scenario over that socket,
then re-pointing `apps/mobile` at the Rust `Core` (then deleting `src/core`)
— the last item before the F2b stop-point.
