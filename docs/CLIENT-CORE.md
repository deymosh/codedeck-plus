...
| in-process binding (Tauri) | `apps/mobile/src/platform/relayTransport.ts` + boot in `src/main.tsx` | `apps/mobile/src-tauri/src/corebridge.rs` | ✅ F1 (Rust side) — `native-core` Cargo feature (off by default → byte-identical build); `core_*` commands + `core://{connection,message,action-failed}` events. WebView opt-in wiring + TS-transport removal ⏳ |
