...
as Tauri commands over real SQLite persistence, and `apps/mobile/src/core`
now has a complete native-backed composition
(`createPhoneCoreNative.ts` + eleven `stores/native*.ts` adapters +
`services/nativeBridgeApi.ts`) presenting the exact same `PhoneCore` shape
`createPhoneCore.ts` does. Nothing calls it yet — the actual cutover
(a `main.tsx` capability check, the `SessionScreen.tsx` image-upload branch,
a full manual smoke pass, then deleting `src/core`) is the remaining item
before the F2b stop-point; see the port-tracking table's last rows for the
two real gaps that surfaced (`removeMachine`, image upload) and are not yet
closed.
...
| Re-point `apps/mobile` at the Rust `Core` + delete `src/core` | `createPhoneCore.ts` consumers | `apps/mobile/src/core/createPhoneCoreNative.ts` | ✅ F2b (composition landed) — `createPhoneCoreNative` assembles all eleven native store adapters (`createNative{Outbox,Settings,Pairing,Machines,Dm,Marmot,QuickPrompts,PendingSessions,Ui,Connection,Transcript}Store`) + `createNativeBridgeApi` into the exact `PhoneCore` shape `createPhoneCore.ts` builds, so `usePhoneCore()` and every screen need zero changes to run against it. `identity.ts` needs no adapter (the TS side always owns loading/generating the persisted secret locally — it is the `core_init` *argument*, not something Rust exposes back). `PhoneCore.client` is now optional (nothing in production UI reads it) and `PhoneCore.api` is typed as `BridgeApiLike`. Two real, documented gaps remain: `removeMachine` (no `Intent` for unpairing a machine exists yet) and image upload (needs an actual `SessionScreen.tsx` code branch calling `dispatch({ sendSessionImage })` directly, not a backend shim — see `createNativeBridgeApi`'s doc). ⏳ Left before the swap is real: which `main.tsx` capability check picks `createPhoneCoreNative` over `createPhoneCore`, the `SessionScreen.tsx` image-upload branch above, a real end-to-end manual smoke pass, and only then deleting `src/core`. |
