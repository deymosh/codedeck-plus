...
export interface SettingsData {
  ...
  /** Route the WebView's relay traffic through Orbot's SOCKS5 proxy
   *  (tauri-plugin-tor-proxy). Off by default — most installs don't run
   *  Orbot; enabling it against a stopped Orbot just makes every relay
   *  connection fail until Orbot is running. Android only; a no-op on desktop. */
...
export function hydrateSettings(raw: string | undefined): SettingsData {
  ...
  try {
    ...
    return {
      ...
      torProxyEnabled:
        typeof parsed.torProxyEnabled === 'boolean'
          ? parsed.torProxyEnabled
          : defaults.torProxyEnabled,
      meshTestTarget:
...
export interface SettingsStoreState extends SettingsData {
  ...
  setTorProxyEnabled(on: boolean): void;
