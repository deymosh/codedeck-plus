...
import com.codedeck.mesh.R
import com.codedeck.mesh.appCoreDataDir
import org.nostrvpn.app.core.NativeCore
import com.codedeck.mesh.seedMobileConfig
...
class MeshVpnService : VpnService() {
    ...
    private fun failStart(
        ...
        if (error == null) {
            Log.w("MeshVpnService", message)
        } else {
            Log.w("MeshVpnService", message, error)
        }
        if (!running.get()) {
            if (foregroundStarted) {
                stopServiceForeground()
    ...
    private fun buildVpnInterface(config: JSONObject): ParcelFileDescriptor? {
        ...
        // SECURITY (split-tunnel hard cap): CodeDeck is a mesh-only dev tool — it must NEVER route
        // the device's whole internet through the tun (that would make it an open exit node and
        // could capture CodeDeck's own relay traffic → deadlock). We therefore route ONLY the mesh
        // CIDR (10.44.0.0/16), dropping anything broader the engine emits (e.g. an accidental
        // 0.0.0.0/0 from an exit-node/WG-upstream mode we don't use). This is defense-in-depth on top
        // of never enabling exit-node mode.
        var addedMeshRoute = false
        val routes = config.optJSONArray("routeTargets")
        if (routes != null) {
            for (index in 0 until routes.length()) {
                val raw = routes.optString(index).trim()
                val route = parseCidr(raw) ?: continue
                if (!isWithinMeshCidr(route)) {
                    Log.w("MeshVpnService", "Dropping non-mesh route $raw (split-tunnel hard cap; mesh-only)")
                    continue
                }
        ...
        // Guarantee the mesh subnet is routed even if the engine emitted nothing usable.
        if (!addedMeshRoute) {
            builder.addRoute("10.44.0.0", 16)
        }
        ...
        return runCatching {
            ...
            Log.w("MeshVpnService", "Failed to establish Android VPN interface", error)
        }.getOrNull()
    }
    ...
    private fun addDnsServers(builder: Builder, config: JSONObject) {
        ...
        for (server in effectiveServers) {
            runCatching {
                ...
                Log.w("MeshVpnService", "Ignoring invalid VPN DNS server: $server", error)
            }
        }
    }
    ...
    /**
     * True only for IPv4 routes contained within the mesh CIDR 10.44.0.0/16. Enforces the split-tunnel
     * hard cap: the route's base must be 10.44.x.x AND its prefix must be >= 16 (so it cannot be a
     * superset like 10.0.0.0/8 or 0.0.0.0/0). Non-IPv4 / unparseable routes are rejected.
     */
    private fun isWithinMeshCidr(cidr: Cidr): Boolean {
        if (cidr.prefix < 16) return false
    ...
    private fun startServiceForeground(): Boolean {
        ...
        return runCatching {
            ...
            Log.w("MeshVpnService", "Failed to start foreground VPN notification", error)
        }.isSuccess
    }
    ...
    private fun publishTunnelNotification() {
        ...
        runCatching {
            ...
            Log.w("MeshVpnService", "Failed to publish VPN notification", error)
        }
    }
    ...
    private fun tunnelNotification(): Notification {
        ...
        val disconnect = PendingIntent.getService(
            this,
            1,
            Intent(this, MeshVpnService::class.java).setAction(ACTION_DISCONNECT),
            PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT,
        )
        return Notification.Builder(this, NOTIFICATION_CHANNEL_ID)
            .setSmallIcon(R.drawable.ic_launcher_monochrome)
            .setContentTitle("CodeDeck mesh")
    ...
    companion object {
        const val ACTION_CONNECT = "com.codedeck.mesh.vpn.CONNECT"
        const val ACTION_DISCONNECT = "com.codedeck.mesh.vpn.DISCONNECT"
        const val ACTION_RESTORE = "com.codedeck.mesh.vpn.RESTORE"
        const val EXTRA_CONFIG_JSON = "configJson"
        private const val NOTIFICATION_CHANNEL_ID = "vpn"
        private const val NOTIFICATION_ID = 7001

        /** Whether the tunnel is currently up. Read by MeshPlugin for status reporting. */
        @Volatile
        var isRunning: Boolean = false
            private set

        fun startRestore(context: Context) {
            val intent = Intent(context, MeshVpnService::class.java)
