...
//! the MLS crypto + group state, persisted in its own SQLCipher-encrypted
//! SQLite file. The caller owns transport: every method returns Nostr events as
//! JSON to publish, and received relay events are fed back in via
//! `MarmotService::ingest_group_message` / `unwrap_welcome`.
//!
//!
