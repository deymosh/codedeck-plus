  ...
  for (const name of names) {
    switch (name) {
      case 'claude-code': {
        ...
        const claudePath = testMode ? null : resolveClaudeExecutable(env.CODEDECK_CLAUDE_PATH);
        if (!testMode && !claudePath) log('[agent-host] claude executable not found — relying on the SDK to locate it');
        drivers.push(
          new ClaudeDriver({
