/**
 * The ONE seam between the Claude driver and `@anthropic-ai/claude-agent-sdk`.
 *
 * The rest of the driver programs against the narrow `SdkFacade` /
 * `SdkSessionHandle` interfaces defined here, so an SDK upgrade only ever
 * touches this file. Driver tests run against a scripted facade, and test
 * mode against `TestModeSdkFacade` — `RealSdkFacade` is deliberately NOT
 * unit-tested: it is kept so thin (pure API-shape adaptation, no business
 * logic) that its only meaningful test would be spawning a real Claude Code
 * subprocess.
 *
 ...
 *  ids (driver.ts); `auto` = the model default. */
 ...
 * to talk to `ANTHROPIC_BASE_URL` (see env.ts's `buildClaudeEnv`
 ...
 * turn-boundary signal the driver derives the turn state from) when
