...
import type { EntryOf, OutputEntry } from '@codedeck/protocol';
...
import { newTranslateContext, type TranslateContext } from '../sdk/adapter';
...
function translate(msg: unknown, ctx: TranslateContext = newTranslateContext()): OutputEntry[] {
  return opencodeMessageToEntries(msg as SdkMessage, ctx);
}

function partMsg(part: unknown, role: 'user' | 'assistant' = 'assistant'): OpenCodePartMessage {
  return { type: 'opencode-part', part: part as Part, role };
}

function toolPart(state: Record<string, unknown>, tool = 'bash', callID = 'call_1'): OpenCodePartMessage {
...
describe('opencodeMessageToEntries', () => {
  describe('system messages', () => {
    it('converts init into a status line naming the model', () => {
      const msg: OpenCodeInitMessage = {
        type: 'system', subtype: 'init', session_id: 'ses_abc',
        model: 'anthropic/claude-sonnet-4-6', permissionMode: 'default',
      };
    ...
    it('converts init with no model without throwing', () => {
      const msg: OpenCodeInitMessage = { type: 'system', subtype: 'init', session_id: 'ses_abc' };
      expect(translate(msg)).toMatchObject([{ entryType: 'status', text: 'OpenCode session started' }]);
    });

    it('converts idle state into turn_complete', () => {
      const msg: OpenCodeStateMessage = { type: 'system', subtype: 'session_state_changed', state: 'idle' };
      expect(translate(msg)).toEqual([{ entryType: 'turn_complete', timestamp: expect.any(String) }]);
    });

    it('drops running state (nothing to relay)', () => {
      const msg: OpenCodeStateMessage = { type: 'system', subtype: 'session_state_changed', state: 'running' };
      expect(translate(msg)).toEqual([]);
    });
  });
  ...
  describe('diff messages', () => {
    ...
    it('emits one entry per file for a multi-file diff event', () => {
      const msg: OpenCodeDiffMessage = { type: 'opencode-diff', files: [fileDiff({ file: 'a.ts' }), fileDiff({ file: 'b.ts' })] };
    ...
    it('falls back to a flat del/add rendering (no LCS) for a file above the line-count guard', () => {
      ...
      const msg: OpenCodeDiffMessage = { type: 'opencode-diff', files: [fileDiff({ file: 'huge.ts', before: bigBefore, after: bigAfter })] };
    ...
    it('a file with only counts (no patch, no before/after) produces no card instead of throwing', () => {
      const msg: OpenCodeDiffMessage = { type: 'opencode-diff', files: [{ file: 'bin.png', additions: 0, deletions: 0 }] };
      expect(translate(msg)).toEqual([]);
    });
  });
  ...
  describe('question messages', () => {
    it('emit nothing themselves, and hide the question tool\'s call', () => {
      ...
      const msg: OpenCodeQuestionMessage = {
        ...
        toolUseId: 'call_q',
        questions: [{ question: 'Which color?', header: 'Color', options: [{ label: 'Red' }], multiSelect: false }],
      };
