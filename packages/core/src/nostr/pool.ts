...
export class BridgePool {
   ...
   *  `idleTimeout` — hence the widened return type instead of a cast. */
  private static poolOptions(): ConstructorParameters<typeof SimplePool>[0] & {
    idleTimeout: number;
  } {
    return {
  ...
  connect(): void {
    ...
    this.pool = new SimplePool(BridgePool.poolOptions());
